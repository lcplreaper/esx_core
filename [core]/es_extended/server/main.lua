...
function onPlayerJoined(playerId)
	...
	if identifier then
		if ESX.GetPlayerFromIdentifier(identifier) then
			...
			MySQL.scalar('SELECT 1 FROM users WHERE identifier = @identifier', {
				...
				if result then
					...
					MySQL.insert('INSERT INTO users (accounts, identifier) VALUES (@accounts, @identifier)', {
...
function loadESXPlayer(identifier, playerId)
	...
	table.insert(tasks, function(cb)
		MySQL.query('SELECT accounts, job, job_grade, `group`, loadout, position, inventory FROM users WHERE identifier = @identifier', {
			['@identifier'] = identifier
		}, function(result)
